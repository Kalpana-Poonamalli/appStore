...
class AppStore extends Component {
  state = {activeButton: tabsList[0].tabId, searchInput: ''}

  changeList = tabId => {
    this.setState({activeButton: tabId})
  }
  ...
  render() {
    const {activeButton, searchInput} = this.state
    const filteredList = appsList.filter(
      eachApp => eachApp.category === activeButton,
    )

    const inputList =
      searchInput.length === 0
        ? filteredList
        : filteredList.filter(eachApp =>
    ...
    return (
      <ul className="app-container">
        ...
        <ul className="buttons-container">
          {tabsList.map(eachButton => (
            <TabItem
              eachButton={eachButton}
              key={eachButton.tabId}
              changeList={this.changeList}
              isActive={eachButton.tabId === activeButton}
            />
          ))}
        </ul>
        <ul className="cards-container">
          {inputList.map(eachApp => (
